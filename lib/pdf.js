...
export default class PDFJSClass extends EventEmitter {
   ...
   parsePDFData(arrayBuffer, password, metadata, infoCallback) {
      this.pdfDocument = null;

      const parameters = { password: password, data: arrayBuffer };
      PDFJS.getDocument(parameters).then(
         (pdfDocument) => { infoCallback(pdfDocument); return this.load(pdfDocument, 1, metadata)} ,
   ...
   load(pdfDocument, scale, metadata) {
   ...
   loadPages(metadata) {
      const pagesCount = metadata ? metadata.to - metadata.from + 1 : this.pdfDocument.numPages;
      const pagePromises = [];

      for (let i = (metadata ? metadata.from : 1); i <= (metadata ? metadata.to : pagesCount); i++){
      ...
      return pagesPromise.then(
         (promisedPages) => this.parsePage(promisedPages, 0, 1.5, metadata),
         (error) => this.raiseErrorEvent('pagesPromise error: ' + error)
      );
   }

   parsePage(promisedPages, id, scale, metadata) {
      ...
      function continueOnNextPage() {
         ...
         const count = (metadata ? metadata.to - metadata.from + 1 : pagesCount) - 1;
         ...
         if (id === count) {
            ...
            process.nextTick(() => this.parsePage(promisedPages, ++id, scale, metadata));
         }
      }
